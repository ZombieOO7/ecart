...
if ((isset($_POST['add-image'])) && ($_POST['add-image'] == 1)) {
    ...
    $type = $db->escapeString($fn->xss_clean($_POST['offer_type']));
    ...
    if (empty($error['image'])) {
        ...
        $sql = "INSERT INTO `offers`(`image`,`position`,`section_position`,`type`,`type_id`,`offer_image_url`) VALUES ('$upload_image','$position','$section_name','$type','$id','$offer_image_url')";
        $db->sql($sql);
        $res = $db->getResult();
