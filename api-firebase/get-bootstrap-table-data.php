...
if (isset($_GET['table']) && $_GET['table'] == 'orders') {
    ...
    foreach ($res as $row) {
        ...
        $tempRow['admin_delivery_charge'] = $row['admin_delivery_charge'];
...
if (isset($_GET['table']) && $_GET['table'] == 'seller') {
    ...
    foreach ($res as $row) {
        ...
        if(isset($row['categories']) && $row['categories'] !=''){
            ...
            for($i=0; $i<count($res); $i++){
                $cate_name = $fn->get_data($column = ['name', 'id'], "id=" . $res[$i], 'category');
