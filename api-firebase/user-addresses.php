                ...
                if (!empty($res)) {
                    ...
                    for ($i = 0; $i < count($res); $i++) {
                        if (isset($res[$i]['area_id']) && !empty($res[$i]['area_id'])) {
                            $area_data = $fn->get_data($columns = ['minimum_free_delivery_order_amount', 'delivery_charges'], 'id=' . $res[$i]['area_id'], 'area');
                        } else {
                            $area_data = $fn->get_data(['minimum_free_delivery_order_amount', 'delivery_charges'], 'name= "' . trim($res[$i]['area'] . '"'), 'area');
                        }
