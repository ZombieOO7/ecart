...
$shiprocket_data = $fn->get_settings('shiprocket', true);
    ...
    if (!empty($user_id)) {
        if ($fn->is_item_available_in_user_cart($user_id)) {
                            ...
                            if ($standard_shipping == 1) {
                                ...
                                if (($pincode_id != 0)) {
                                    ...
                                    if (!empty($user_pincode)) {
                                        ...
                                        $pincodes_data = $fn->get_data($column = ['*'], "pincode=" . $pincode_id, "pincodes");
                                        // print_r($pincodes_data);
                        ...
                        if (!empty($pincode_id)) {
                            ...
                            if ($res[$i]['item'][$k]['d_type'] == "all") {
                                ...
                                if (in_array($pincode_id, $pincodes)) {
                                    $res[$i]['item'][$k]['is_item_deliverable']  = true;
                                    // print_r($res[$i]['item'][$k]['is_item_deliverable']);  
                                } else {
                                    $res[$i]['item'][$k]['is_item_deliverable']  = false;
                                }
            ...
            if ($shipping_type == 'standard') {
                ...
                // print_r($delivery_charge_parcels);
                ...
                if (!empty($type) && $type == 'delivery_charge') {
                    ...
                    if ($shipping_type == 'standard') {
                        if (isset($shiprocket_data['free_delivery_charge']) && $shiprocket_data['free_delivery_charge'] == "1") {
                            if ($sub_total >= $shiprocket_data['minimum_oredr_amount']) {
                                $response['delivery_charge_with_cod'] = 0;
                                $response['delivery_charge_without_cod'] = 0;
                            } else {
