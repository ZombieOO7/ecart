...
if ($res_outer[0]['address_id'] != 0) {
    $user_address = $function->get_data(['area', 'pincode', 'city', 'name', 'mobile'], 'id=' . $res_outer[0]['address_id'], 'user_addresses');
}
    ...
    <section class="invoice">
        ...
        <div class="row invoice-info">
            ...
            <div class="col-sm-5 invoice-col">
                Shipping Address
                <address>
                    <strong><?= $user_address[0]['name']; ?></strong>
                </address>
                <address>
                ...
                <address>
                    <strong><?= $user_address[0]['mobile']; ?></strong><br>
                </address>
                <address>
        ...
        <?php for ($i = 0; $i < count($seller_ids); $i++) {
            ...
            foreach ($res_items as $row) {
                ...
                $data = array($row['product_id'], $row['pname'], $row['quantity'], $row['measurement'], $row['mesurement_unit_name'], $row['discounted_price'] * $row['quantity'], $row['discount'], $row['sub_total'], $row['order_item_status'], $row['tax_id'], $final_price, $row['seller_id'], $row['hsn_code']);
                // print_r($data);
