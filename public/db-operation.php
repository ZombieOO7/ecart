    ...
    if ($_FILES["update_file"]["size"] > 0  && $error == false) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if ($count != 0) {
                ...
                if (!empty($emapData[2])) {
                    $seller = $fn->get_data($columns = ['name'], "id=" . $emapData[12], 'seller');
                    if (empty($seller)) {
                ...
                if (!empty($emapData[17])) {
                    $tax = $fn->get_data(['id'], "id = " . $emapData[17], 'taxes');
                    if (empty($tax)) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if ($count1 != 0) {
                ...
                $db->update('products', $data, 'id=' . $emapData[0]);
                $res = $db->getResult();
                ...
                for ($i = 0; $i < $total_variants; $i++) {
                    ...
                    $db->update('product_variant', $variant_data[$i], 'id=' . $emapData[20]);
                    $res1 = $db->getResult();
                }
            }
            $count1++;
